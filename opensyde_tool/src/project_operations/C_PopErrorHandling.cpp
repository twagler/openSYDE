                                          ...
                                          const std::vector<stw::scl::C_SclString> & orc_ErrorDetailsMissingDevices)
{
   ...
   else if ((ors32_Err != C_NO_ERR) && (ors32_Err != C_WARN))
   {
      ...
      c_Description = "Failed to load project: " + orc_Path;
      c_Message.SetCustomMinHeight(250, 425);

      switch (ors32_Err)
      {
      case C_RD_WR:
         c_Details = "There are problems accessing the file system.<br/>"
                                            "For example, there may be no read access to the file.";
         break;
      case C_RANGE:
         c_Details = "At least one project file is missing.";
         break;
      case C_NOACT:
         c_Details = "A project file is present but its structure is invalid.<br/>"
                                            "For example this can be caused by an invalid XML file.";
         break;
      case C_CONFIG:
         c_Details = "The content of a project file is invalid or incomplete.";
         break;
      case C_CHECKSUM:
         c_Details = "UI part does not match core part.";
         break;
      case C_COM:
         ...
         break;
      case C_BUSY:
         c_Details = "Problems with cleaning up temporary folders.";
         break;
      default:
         c_Details = "Unknown cause.";
         break;
      }
   ...
   else
   {
          ...
          (orc_Path.isEmpty() == false))
      {
         ...
         c_Message.SetHeading("Project load");
         c_Message.SetDescription("Loading an older project version. On the next save the "
      ...
      case C_RD_WR:
         {
            ...
            c_Details += c_LogLink;
            c_Message.SetDescription("Problems accessing file system.");
            c_Message.SetDetails(c_Details);
            break;
         }
      case C_RANGE:
         c_Message.SetDescription("Path is empty.");
         break;
      case C_NOACT:
         c_Message.SetDescription("Could not create project directory.");
         break;
      default:
         c_Message.SetDescription("Unknown cause.");
         break;
      }
...
void C_PopErrorHandling::h_ServiceProjectSaveErr(const int32_t & ors32_Err, QWidget * const opc_Parent)
{
   if (ors32_Err != C_NO_ERR)
   {
      ...
      {
      case C_RANGE:
         c_Message.SetDescription("Invalid view configuration.");
         break;
      case C_RD_WR:
         c_Message.SetDescription("Problems accessing file system.");
         break;
      case C_NOACT:
         c_Message.SetDescription("Could not create temporary project directory.");
         break;
      case C_BUSY:
         c_Message.SetDescription("Could not delete temporary folder.");
         break;
      case C_COM:
         c_Message.SetDescription("Bus sorting failed.");
         break;
      case C_WARN:
         c_Message.SetDescription("No view or part of it is active.");
         break;
      case C_CONFIG:
         c_Message.SetDescription("Expected file for project generation missing.");
         break;
      default:
         c_Message.SetDescription("Unknown cause.");
         break;
...
void C_PopErrorHandling::h_GetTextInitializeErr(const int32_t & ors32_Err)
{
   if (ors32_Err != C_NO_ERR)
   {
      ...
      {
      case C_CONFIG:
         pc_Widget->SetErrorDescription("C_OscGetText - h_Initialize:\n"
      ...
      case C_NOACT:
         ...
         break;
      default:
         pc_Widget->SetErrorDescription("C_OscGetText - h_Initialize: unknown error");
         c_Dialog.exec();
         break;
      }
   }
