                                               ...
                                               std::vector<C_OscNodeDataPool> * const opc_OscDataPools)
{
   ...
   if (orc_XmlParser.SelectNodeChild("data-pools") == "data-pools")
   {
      ...
      //Return
      Q_ASSERT(orc_XmlParser.SelectNodeParent() == "gui");
   }
   else
   {
      s32_Retval = C_CONFIG;
...
void C_PuiSdHandlerFilerV2::h_SaveNodes(const std::vector<C_PuiSdNode> & orc_Nodes, C_OscXmlParserBase & orc_XmlParser)
{
   if (orc_Nodes.size() > 0)
   {
      ...
      Q_ASSERT(orc_XmlParser.SelectNodeChild("node") == "node"); //first node ...
      for (uint32_t u32_Index = 0U; u32_Index < orc_Nodes.size(); u32_Index++)
      {
         ...
         {
            Q_ASSERT(orc_XmlParser.SelectNodeNext("node") == "node"); //next node
         }
      }
      Q_ASSERT(orc_XmlParser.SelectNodeParent() == "nodes"); //back up
   }
}
...
void C_PuiSdHandlerFilerV2::h_SaveBuses(const std::vector<C_PuiSdBus> & orc_Buses, C_OscXmlParserBase & orc_XmlParser)
{
   if (orc_Buses.size() > 0)
   {
      ...
      Q_ASSERT(orc_XmlParser.SelectNodeChild("bus") == "bus"); //first bus ...
      ...
      Q_ASSERT(orc_XmlParser.SelectNodeParent() == "buses"); //back up
   }
}
                                                     ...
                                                     C_OscXmlParserBase & orc_XmlParser)
{
   ...
   if (c_SelectedNode == "bus-text-element")
   {
      ...
      //Return
      Q_ASSERT(orc_XmlParser.SelectNodeParent() == "bus-text-elements");
   }
   //Compare length
   if ((s32_Retval == C_NO_ERR) && (q_ExpectedSizeHere == true))
   {
                                                  ...
                                                  C_OscXmlParserBase & orc_XmlParser)
{
   ...
   for (uint32_t u32_Index = 0U; u32_Index < orc_BusTextElements.size(); ++u32_Index)
   {
      ...
      Q_ASSERT(orc_XmlParser.SelectNodeParent() == "bus-text-elements"); //back up
   }
}
         ...
         if (s32_Return == C_NO_ERR)
         {
            ...
            if ((s32_Return == C_NO_ERR) && (orc_XmlParser.SelectNodeChild("busconnections") == "busconnections"))
            {
               ...
               if (orc_XmlParser.SelectNodeChild("connection") == "connection")
               {
...

                        {
                           ...
                           if (orc_XmlParser.SelectNodeChild("interaction-points") == "interaction-points")
                           {
                              ...
                              if (c_Text == "interaction-point")
                              {
                                 ...
                                 Q_ASSERT(orc_XmlParser.SelectNodeParent() == "interaction-points");
                              }
                  ...
                  Q_ASSERT(orc_XmlParser.SelectNodeParent() == "busconnections"); //back to busconnections
               }
...
void C_PuiSdHandlerFilerV2::mh_SaveNode(const C_PuiSdNode & orc_Node, C_OscXmlParserBase & orc_XmlParser)
{
   ...
   for (uint16_t u16_Bus = 0U; u16_Bus < orc_Node.c_UiBusConnections.size(); u16_Bus++)
   {
      ...
      Q_ASSERT(orc_XmlParser.SelectNodeParent() == "connection");     //back to connection
      Q_ASSERT(orc_XmlParser.SelectNodeParent() == "busconnections"); //back to busconnections
   }

   Q_ASSERT(orc_XmlParser.SelectNodeParent() == "gui");  //back to "gui"
   Q_ASSERT(orc_XmlParser.SelectNodeParent() == "node"); //back to "node"
}
