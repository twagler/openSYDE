...
void C_SyvComDriverDiagConnect::m_RunSetDiagnosticMode(void)
{
   ...
   if (this->mpc_ComDriverDiag != NULL)
   {
      ...
      case C_NO_ERR:
         break;
      case C_CONFIG:
         this->mc_ErrorMessage = static_cast<QString>("Initialization was not finished.");
         break;
      case C_NOACT:
         this->mc_ErrorMessage =
            static_cast<QString>("System View is invalid. Action cannot be performed.");
         break;
      case C_TIMEOUT:
         this->mc_ErrorMessage = static_cast<QString>("At least one node is not responding!");
         this->mc_ErrorMessageDetails =
            static_cast<QString>("Affected node(s):\n%1").arg(c_ErrorDetails);
         break;
      case C_BUSY:
         this->mc_ErrorMessage =
            static_cast<QString>("Connecting to at least one node failed!");
         this->mc_ErrorMessageDetails =
            static_cast<QString>("Affected node(s):\n%1\n\n"
                                                        "Possible scenarios:\n"
      ...
      case C_DEFAULT:
         this->mc_ErrorMessage =
            static_cast<QString>("The Datapool definition differs between client and node.\n"
         ...
         this->mc_ErrorMessageDetails =
            static_cast<QString>("Affected node(s) and Datapool(s):\n%1").arg(
               c_ErrorDetails);
         break;
      case C_COM:
         this->mc_ErrorMessage = static_cast<QString>("Service error response detected.");
         this->mc_ErrorMessageDetails =
            static_cast<QString>("Affected node(s):\n%1\n\n"
                                                        "Possible scenario:\n"
         ...
         break;
      case C_RD_WR:
         this->mc_ErrorMessage = static_cast<QString>("Unexpected protocol response detected.");
         this->mc_ErrorMessageDetails = static_cast<QString>("Possible scenario:\n"
                                                                                    "If there are multiple nodes with the same node ID connected to the same bus");
         break;
      case C_WARN:
         this->mc_ErrorMessage = static_cast<QString>("Protocol error response detected.");
         this->mc_ErrorMessageDetails = static_cast<QString>("Possible scenario:\n"
      ...
      case C_CHECKSUM:
         this->mc_ErrorMessage =
            static_cast<QString>("Authentication between openSYDE Tool and device(s) has failed. Access denied.");
         C_OscLoggingHandler::h_Flush();
         this->mc_ErrorMessageDetails = "Possible reasons:<br/>"
      ...
      default:
         this->mc_ErrorMessage =
            static_cast<QString>("Unknown error: %1").arg(C_Uti::h_StwError(this->
   ...
   if (this->mpc_ComDriverDiag != NULL)
   {
      ...
      case C_NO_ERR:
         if (c_FailedIdRegisters.size() > 0)
         {
            ...
            this->mc_ErrorMessage =
               static_cast<QString>("Not all cyclic transmission could be registered. (%1 failed)").
               arg(
      ...
      case C_CONFIG:
         this->mc_ErrorMessage =
            static_cast<QString>("System connection failed. Configuration error.");
         //Many  possibilities for this error
         break;
      case C_COM:
         this->mc_ErrorMessage =
            static_cast<QString>("Communication error. (%1)").arg(c_ErrorDetails);
         break;
      default:
         this->mc_ErrorMessage =
            static_cast<QString>("Unknown error: %1").
